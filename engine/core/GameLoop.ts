...
import { MapSystem } from '../systems/MapSystem'; // IMPORT MAP SYSTEM
...
export class GameLoop {
    ...
    public start() {
        ...
        EnemyHealthBar.init(); 
        MapSystem.init(); // Init Fog of War
        ...
        GameDirector.setResetCallback(() => {
            MapSystem.init(); // Reset map on new level
            this.loadLevel();
            BossAI.reset(); 
        });
    ...
    private update(dt: number, time: number) {
        ...
        // MAP DISCOVERY
        if (this.frameCount % 5 === 0) { // Optimize: Only check every 5 frames
            MapSystem.explore(this.player.position);
        }
        ...
        if (this.frameCount % 10 === 0) {
            ...
            // MINIMAP UPDATE
            // Note: We access the private array from lootSystem via public accessor or direct access if public
            // For now assuming LootSystem has public items accessor or we pass it
